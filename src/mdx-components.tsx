...
const components = {
  h1: ({ className, children, ...props }: HeadingProps) => {
    ...
    return (
      <h1
        id={id}
        className={cn(
          "lg:text-3xl text-2xl font-bold mb-4 scroll-mt-16",
          className
        )}
        {...props}
      >
  ...
  h2: ({ className, children, ...props }: HeadingProps) => {
    ...
    return (
      <h2
        id={id}
        className={cn(
          "lg:text-2xl text-xl font-semibold mb-4 mt-4 scroll-mt-16",
          className
        )}
        {...props}
      >
  ...
  h3: ({ className, children, ...props }: HeadingProps) => {
    ...
    return (
      <h3
        id={id}
        className={cn(
          "lg:text-lg font-medium mb-3 mt-4 scroll-mt-16",
          className
        )}
        {...props}
      >
  ...
  h4: ({ className, children, ...props }: HeadingProps) => {
    ...
    return (
      <h4
        id={id}
        className={cn(
          "lg:text-base font-medium mb-2 mt-4 scroll-mt-16",
          className
        )}
        {...props}
      >
  ...
  p: ({ className, children, ...props }: ParagraphProps) => (
    <p className={cn("mb-4 mt-0 text-muted-foreground", className)} {...props}>
      {children}
    </p>
  ),
  ...
  table: ({ className, children, ...props }: TableProps) => (
    <div className="overflow-x-auto">
      <table
        className={cn("w-full border-collapse md:!my-4 !my-2", className)}
        {...props}
      >
        {children}
      </table>
  ...
  img: ({ className, alt, ...props }: ImgProps) => (
    <img className={cn("rounded-md my-4", className)} alt={alt} {...props} />
  ),
};
