...
export default function TableOfContents({content}: TableOfContentsProps) {
        ...
        {items.map((tocItem) => (
          <motion.li key={tocItem.id} variants={item}>
            ...
            {tocItem.children && tocItem.children.length > 0 && (
              <motion.ul className="ml-4 mt-2 space-y-2" variants={container} initial="hidden" animate="show">
  ...
  return (
    <aside
      className="hidden no-scrollbar h-[calc(100vh_-_70px)] top-[70px] left-0 right-0 lg:block w-64 shrink-0 border-l border-border sticky overflow-y-auto py-4 pl-4"
    >
      <motion.div className="space-y-6" variants={container} initial="hidden" animate="show">
        <div>
          ...
          {tocItems.length > 0 ? (
            ...
            <p className="text-muted-foreground">No headings found in this document.</p>
          )}
        </div>
